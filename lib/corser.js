...
exports.create = function (options) {
    ...
    options.requestHeaders = options.hasOwnProperty("requestHeaders") ? toLowerCase(options.requestHeaders) : simpleRequestHeaders;
    options.responseHeaders = options.hasOwnProperty("responseHeaders") ? toLowerCase(options.responseHeaders) : simpleResponseHeaders;
    options.maxAge = options.maxAge || null;
    options.supportsCredentials = options.supportsCredentials || false;
    return function (req, res, next) {
        var originMatches, methodMatches, headersMatch, requestMethod
                    ...
                    if (!req.headers.hasOwnProperty("access-control-request-method")) {
                        ...
                        if (methodMatches === false) {
                            ...
                            requestHeaders.forEach(function (requestHeader) {
                                ...
                                // Browsers automatically add Origin to Access-Control-Request-Headers. However, Origin is not one of the
                                // simple request headers. Therefore, the header is accepted even if it is not in the list of request
                                // headers because CORS would not work without it.
